...
public:
    ...
    template <class Y>
    SharedPtr(const SharedPtr<Y, ThreadSafe>& r, element_type* ptr) noexcept
    {
        AliasCopyFrom(r, ptr);
    }

    template <class Y>
    SharedPtr(SharedPtr<Y, ThreadSafe>&& r, element_type* ptr) noexcept
    {
    ...
    template <class Y>
    SharedPtr(const SharedPtr<Y, ThreadSafe>& r) noexcept
    {
        CopyFrom(r);
    }
    ...
    template <class Y>
    SharedPtr(SharedPtr<Y, ThreadSafe>&& r) noexcept
    {
        MoveFrom(std::move(r));
    }

    template <class Y>
    explicit SharedPtr(const WeakPtr<Y, ThreadSafe>& r)
    {
        if (!FromWeak(r))
    ...
    template <class U>
    constexpr bool operator==(const SharedPtr<U, ThreadSafe>& r) const noexcept
    {
        return ptr_ == r.ptr_;
    }
...
public:
    ...
    template <class Y>
    WeakPtr(const WeakPtr<Y, ThreadSafe>& r) noexcept
    {
        CopyFrom(r);
    }

    template <class Y>
    WeakPtr(const SharedPtr<Y, ThreadSafe>& r) noexcept
    {
        CopyFrom(r);
    }
    ...
    template <class Y>
    WeakPtr(WeakPtr<Y, ThreadSafe>&& r) noexcept
    {
        MoveFrom(std::move(r));
    }
...
{
    return {std::move(r), static_cast<T*>(r.get())};
}
...
template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> StaticCast(const SharedRef<U, ThreadSafe>& r) noexcept
{
    return {r, static_cast<T*>(r.get())};
}

template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> StaticCast(SharedRef<U, ThreadSafe>&& r) noexcept
{
    return {std::move(r), static_cast<T*>(r.get())};
}

template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> ConstCast(const SharedRef<U, ThreadSafe>& r) noexcept
{
    return {r, const_cast<T*>(r.get())};
}

template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> ConstCast(SharedRef<U, ThreadSafe>&& r) noexcept
{
    return {std::move(r), const_cast<T*>(r.get())};
}

/**
 * @throw std::bad_cast if failed.
 */
template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> DynamicCast(const SharedRef<U, ThreadSafe>& r) noexcept
{
    return {r, &dynamic_cast<T&>(*r)};
}
 ...
 */
template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> DynamicCast(SharedRef<U, ThreadSafe>&& r) noexcept
{
    return {std::move(r), &dynamic_cast<T&>(*r)};
}

template <class T, class U, bool ThreadSafe>
SharedRef<T, ThreadSafe> ReinterpretCast(const SharedRef<U, ThreadSafe>& r) noexcept
{
    return {r, reinterpret_cast<T*>(r.get())};
}
...
template <class T, bool ThreadSafe>
void swap(SharedPtr<T, ThreadSafe>& l, SharedPtr<T, ThreadSafe>& r) noexcept
{
    l.swap(r);
}

template <class T, bool ThreadSafe>
void swap(WeakPtr<T, ThreadSafe>& l, WeakPtr<T, ThreadSafe>& r) noexcept
{
    l.swap(r);
}
