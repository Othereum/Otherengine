...
String ReadFile(const fs::path& path)
{
	...
	if (!file.is_open()) Throw(u8"Cannot read file. File not found: {}"sv, path.string<char8_t>(PoolAllocator<char8_t>{}));

	String code(file.tellg(), '\0');
	file.seekg(0);
...
static auto Ext(fs::path path, std::u8string_view ext)
{
	return path += ext;
}
