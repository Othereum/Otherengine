...
{
	static std::u8string_view FModErrorString(FMOD_RESULT result);
	
	template <class... Args>
	[[noreturn]] static void FModThrow(FMOD_RESULT result, std::u8string_view message, const Args&... args)
	{
...
#define FMOD_CHECK(result, message, ...) (void)(result != FMOD_OK && (FModThrow(result, message, ##__VA_ARGS__), false))
		...
		:system_{}, core_system_{}
	{
		...
		FMOD_CHECK(result, u8"Failed to create FMOD system"sv);

		result = system_->initialize(512, FMOD_STUDIO_INIT_NORMAL, FMOD_INIT_NORMAL, nullptr);
		FMOD_CHECK(result, u8"Failed to initialize FMOD system"sv);
	...
	std::u8string_view FModErrorString(FMOD_RESULT result)
	{
		...
		case FMOD_ERR_BADCOMMAND:
			return u8"Tried to call a function on a data type that does not allow this type of functionality (ie calling Sound::lock on a streaming sound)."sv;
		...
		case FMOD_ERR_DSP_CONNECTION:
			return u8"DSP connection error. Connection possibly caused a cyclic dependency or connected dsps with incompatible buffer counts."sv;

		case FMOD_ERR_DSP_DONTPROCESS:
			return u8"DSP return code from a DSP process query callback. Tells mixer not to call the process callback and therefore not consume CPU. Use this to optimize the DSP graph."sv;

		case FMOD_ERR_DSP_FORMAT:
			return u8"DSP Format error. A DSP unit may have attempted to connect to this network with the wrong format, or a matrix may have been set with the wrong size if the target unit has a specified channel map."sv;
		...
		case FMOD_ERR_DSP_SILENCE:
			return u8"DSP return code from a DSP process query callback. Tells mixer silence would be produced from read, so go idle and not consume CPU. Use this to optimize the DSP graph."sv;
		...
		case FMOD_ERR_FILE_COULDNOTSEEK:
			return u8"Couldn't perform seek operation. This is a limitation of the medium (ie netstreams) or the file format."sv;
		...
		case FMOD_ERR_HEADER_MISMATCH:
			return u8"There is a version mismatch between the FMOD header and either the FMOD Studio library or the FMOD Core library."sv;
		...
		case FMOD_ERR_MEMORY_CANTPOINT:
			return u8"Can't use FMOD_OPENMEMORY_POINT on non PCM source data, or non mp3/xma/adpcm data if FMOD_CREATECOMPRESSEDSAMPLE was used."sv;
		...
		case FMOD_ERR_OUTPUT_ALLOCATED:
			return u8"Error initializing output device, but more specifically, the output device is already in use and cannot be reused."sv;

		case FMOD_ERR_OUTPUT_CREATEBUFFER:
			return u8"Error creating hardware sound buffer."sv;

		case FMOD_ERR_OUTPUT_DRIVERCALL:
			return u8"A call to a standard soundcard driver failed, which could possibly mean a bug in the driver or resources were missing or exhausted."sv;
		...
		case FMOD_ERR_OUTPUT_NODRIVERS:
			return u8"The output device has no drivers installed. If pre-init, FMOD_OUTPUT_NOSOUND is selected as the output mode. If post-init, the function just fails."sv;
		...
		case FMOD_ERR_REVERB_CHANNELGROUP:
			return u8"Reverb properties cannot be set on this channel because a parent channelgroup owns the reverb connection."sv;

		case FMOD_ERR_REVERB_INSTANCE:
			return u8"Specified instance in FMOD_REVERB_PROPERTIES couldn't be set. Most likely because it is an invalid instance number or the reverb doesn't exist."sv;

		case FMOD_ERR_SUBSOUNDS:
			return u8"The error occurred because the sound referenced contains subsounds when it shouldn't have, or it doesn't contain subsounds when it should have. The operation may also not be able to be performed on a parent sound."sv;

		case FMOD_ERR_SUBSOUND_ALLOCATED:
			return u8"This subsound is already being used by another sound, you cannot have more than one parent to a sound. Null out the other parent's entry first."sv;

		case FMOD_ERR_SUBSOUND_CANTMOVE:
			return u8"Shared subsounds cannot be replaced or moved from their parent stream, such as when the parent stream is an FSB file."sv;
		...
		case FMOD_ERR_TOOMANYCHANNELS:
			return u8"The sound created exceeds the allowable input channel count. This can be increased using the 'maxinputchannels' parameter in System::setSoftwareFormat."sv;
		...
		case FMOD_ERR_UNSUPPORTED:
			return u8"A command issued was not supported by this object. Possibly a plugin without certain callbacks specified."sv;
