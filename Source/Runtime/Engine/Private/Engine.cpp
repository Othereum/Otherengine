...
InitEngine::InitEngine(Engine* e)
{
    ...
    OE_LOG(kEngine, kInfo, u8"Initializing engine..."sv);
...
{
    OE_LOG(kEngine, kInfo, u8"Engine initialization successful."sv);
    OE_LOG(kEngine, kInfo, u8"Loading game module..."sv);
    GetGameDll().Call<void(Engine&)>(u8"GameMain"sv, *this);
    OE_LOG(kEngine, kInfo, u8"Game module loaded."sv);
}
...
void Engine::RunLoop()
{
    ...
    OE_LOG(kEngine, kInfo, u8"Engine loop started."sv);
    ...
    OE_LOG(kEngine, kInfo, u8"Average fps: {:.0f}, frame time: {:.2f} ms"sv, ticks_ / sec, ms / ticks_);
}

void Engine::Shutdown()
{
    is_running_ = false;
    OE_LOG(kEngine, kInfo, u8"Engine shutdown requested."sv);
}
